...
static int read_exact_at(int fd, off_t offset, void *buf, size_t len) {
    if (pread(fd, buf, len, offset) != (ssize_t)len) return -1;
    return 0;
}

static uint64_t read_be64_at(int fd, off_t offset) {
    uint8_t b[8];
    if (read_exact_at(fd, offset, b, 8) != 0) return 0;
    uint64_t v = 0;
    for (int i = 0; i < 8; i++) v = (v << 8) | b[i];
    return v;
}

static uint32_t read_be32_at(int fd, off_t offset) {
    uint8_t b[4];
    if (read_exact_at(fd, offset, b, 4) != 0) return 0;
    uint32_t v = 0;
    for (int i = 0; i < 4; i++) v = (v << 8) | b[i];
    return v;
}

/* Writes the raw VBMeta blob of a single partition to `out`. Handles both
 * formats AVB uses: partitions with their own AVB header at offset 0
 * ("AVB0", e.g. vbmeta/vbmeta_system/vbmeta_vendor) and partitions whose
 * VBMeta blob is pointed to by a 64-byte AVB footer at the very end of the
 * partition ("AVBf", e.g. boot/dtbo/recovery). */
static int emit_vbmeta_blob(const char *dev_path, FILE *out) {
    int fd = open(dev_path, O_RDONLY);
    if (fd < 0) return -1;

    off_t sz = lseek(fd, 0, SEEK_END);
    if (sz < 64) { close(fd); return -1; }

    uint64_t vb_off = 0, vb_sz = 0;
    int have_footer = 0;

    uint8_t magic[4];
    if (read_exact_at(fd, sz - 64, magic, 4) == 0 && memcmp(magic, "AVBf", 4) == 0) {
        vb_off = read_be64_at(fd, sz - 64 + 20);
        vb_sz = read_be64_at(fd, sz - 64 + 28);
        have_footer = 1;
    }

    if (!have_footer) {
        uint8_t hdr[4];
        if (read_exact_at(fd, 0, hdr, 4) != 0 || memcmp(hdr, "AVB0", 4) != 0) {
            close(fd);
            return -1;
        }
        uint64_t auth = read_be64_at(fd, 12);
        uint64_t aux = read_be64_at(fd, 20);
        vb_off = 0;
        vb_sz = 256 + auth + aux;
    }

    uint8_t buf[8192];
    uint64_t remaining = vb_sz;
    off_t pos = (off_t)vb_off;
    int ok = 0;
    while (remaining > 0) {
        size_t chunk = remaining < sizeof(buf) ? (size_t)remaining : sizeof(buf);
        ssize_t r = pread(fd, buf, chunk, pos);
        if (r <= 0) { ok = -1; break; }
        if (fwrite(buf, 1, (size_t)r, out) != (size_t)r) { ok = -1; break; }
        pos += r;
...
/* Walks the aux-data hash-descriptor chain of a top-level VBMeta partition
 * and appends the VBMeta blob of every chained partition (tag==4 entries,
 * e.g. boot/dtbo/recovery/vbmeta_system/vbmeta_vendor) to `out` after the
 * top-level blob itself — matching how AVB computes the device's real
 * verified-boot digest (Android's `verifiedBootHash` attestation field).
 * Hashing only the top-level vbmeta blob (the old approach) only matches
 * that value on devices with no chained partitions at all. */
static void walk_vbmeta_chain(const char *dev_path, const char *slot, FILE *out) {
    emit_vbmeta_blob(dev_path, out);

    int fd = open(dev_path, O_RDONLY);
    if (fd < 0) return;

    uint64_t auth = read_be64_at(fd, 12);
    uint64_t desc_off = read_be64_at(fd, 96);
    uint64_t desc_sz = read_be64_at(fd, 104);
    uint64_t aux_start = 256 + auth;

    uint64_t pos = aux_start + desc_off;
    uint64_t end = aux_start + desc_off + desc_sz;
    while (pos < end) {
        uint64_t tag = read_be64_at(fd, (off_t)pos);
        uint64_t nbf = read_be64_at(fd, (off_t)(pos + 8));
        if (tag == 4) {
            uint32_t name_len = read_be32_at(fd, (off_t)(pos + 20));
            if (name_len > 0 && name_len < 256) {
                char name[256];
                if (read_exact_at(fd, (off_t)(pos + 92), name, name_len) == 0) {
                    name[name_len] = '\0';
                    char cand[512];
                    struct stat st;
                    snprintf(cand, sizeof(cand), "/dev/block/by-name/%s%s", name, slot);
                    if (stat(cand, &st) != 0) {
                        snprintf(cand, sizeof(cand), "/dev/block/by-name/%s", name);
                    }
                    if (stat(cand, &st) == 0) {
                        emit_vbmeta_blob(cand, out);
                    }
                }
            }
        }
...
int calculate_vbmeta_digest(char *hash_out, size_t out_len) {
    ...
    char tmp_path[] = "/data/local/tmp/.rod_vbmeta_XXXXXX";
    int tmp_fd = mkstemp(tmp_path);
    if (tmp_fd < 0) return -1;
    FILE *tmp_fp = fdopen(tmp_fd, "wb");
    if (!tmp_fp) { close(tmp_fd); unlink(tmp_path); return -1; }

    walk_vbmeta_chain(dev_path, slot, tmp_fp);
    fclose(tmp_fp);

    char cmd[768];
    snprintf(cmd, sizeof(cmd), "sha256sum \"%s\" | cut -d' ' -f1", tmp_path);
...
void apply_uname_spoof() {
    ...
    if (strcmp(kver, "default") == 0 || strlen(kver) == 0) {
        /* Read the release from /proc/version rather than "uname -r": SuSFS's
         * uname spoof only hooks the uname() syscall's return value, it does
         * not touch /proc/version's banner, so /proc/version stays a pristine
         * baseline even after a spoof has already been applied once. Using
         * "uname -r" here would read back our own (or a prior) fake value on
         * every subsequent call and keep stacking fake suffixes onto it. */
        char current[128] = "";
        FILE *fp = fopen("/proc/version", "r");
        if (fp) {
            char banner[512] = "";
            fgets(banner, sizeof(banner), fp);
            fclose(fp);
            char *start = strstr(banner, "Linux version ");
            if (start) {
                start += 14;
                char *end = start;
                while (*end && *end != ' ') end++;
        ...
        /* Strip any real build suffix (e.g. "-dirty", "-g<hash>") already
         * present on this device's actual kernel release, so our synthetic
         * "-g<fake_hash>" below is never appended on top of a real one. */
        size_t curlen = strlen(current);
        if (curlen > 6 && strcmp(current + curlen - 6, "-dirty") == 0) {
            current[curlen - 6] = '\0';
        }
        char *real_hash = strstr(current, "-g");
        if (real_hash) {
            char *h = real_hash + 2;
            int is_hash = strlen(h) >= 6;
            for (char *c = h; *c && is_hash; c++) {
                if (!isxdigit((unsigned char)*c)) is_hash = 0;
            }
            if (is_hash) *real_hash = '\0';
        }
        ...
        if (major > 5 || (major == 5 && minor >= 10)) {
            ...
            /* Persist the derived value so future calls (this function gets
             * replayed on every unrelated toggle flip) short-circuit via the
             * get_cfg_str() check above instead of re-deriving it every time. */
            set_cfg_str("kernel_version", kver);
        } else {
            strcpy(kver, "default");
        }
    }
...
/* Scans /proc/1/mountinfo for bind mounts leaked into app data by module/overlay
 * systems (e.g. KSU modules, magic mount, overlayfs) that land under /data/app,
 * and hides each one via sus_path so app-side mount enumeration doesn't see it. */
void apply_auto_hide_app_mounts() {
    FILE *fp = fopen("/proc/1/mountinfo", "r");
    if (!fp) return;
    ...
    while (fgets(line, sizeof(line), fp)) {
        ...
        /* mountinfo fields: ID parentID major:minor root mountpoint opts... - fstype source ... */
        char *fields[16];
        int n = 0;
        char *save = line;
        char *tok;
        while (n < 16 && (tok = strtok_r(save, " ", &save)) != NULL) {
            fields[n++] = tok;
        }
        if (n < 4) continue;
        const char *root_field = fields[3];
        const char *mountpoint = fields[4];

        /* Only care about mounts landing under /data/app. */
...
void apply_susfs_post_fs_data() {
    ...
    susfs_set_avc_log_spoofing(get_cfg("spoof_avc_log"));
...
void apply_susfs_boot_completed() {
    ...
    if (get_cfg("auto_hide_app_mounts") == 1) {
        apply_auto_hide_app_mounts();
    }
...
void handle_command(int argc, char *args[]) {
    ...
    if (strcmp(args[0], "susfs") == 0) {
        if (argc >= 2 && strcmp(args[1], "status") == 0) {
            ...
            char kver_buf[128];
            char kbuild_buf[256];
            get_cfg_str("kernel_version", kver_buf, sizeof(kver_buf));
            get_cfg_str("kernel_build", kbuild_buf, sizeof(kbuild_buf));
            snprintf(response, sizeof(response),
                     ...
                     get_cfg("spoof_avc_log") ? "true" : "false",
                     get_cfg("auto_hide_app_mounts") ? "true" : "false",
                     kver_buf,
                     kbuild_buf);
            ...
            if (check_susfs()) {
                ...
                } else if (strcmp(key, "spoof_avc_log") == 0) {
                    susfs_set_avc_log_spoofing(val);
                } else if (strcmp(key, "auto_hide_app_mounts") == 0) {
                    if (val) apply_auto_hide_app_mounts();
                } else {
        ...
        } else if (argc >= 2 && strcmp(args[1], "get_patch") == 0) {
            FILE *fp = fopen(sp_txt, "r");
            if (fp) {
                size_t r = fread(response, 1, sizeof(response) - 1, fp);
                response[r] = '\0';
                fclose(fp);
        ...
        } else if (argc >= 3 && strcmp(args[1], "set_patch") == 0) {
            mkdir(ts_dir, 0755);
            FILE *fp = fopen(sp_txt, "w");
            if (fp) {
                fprintf(fp, "all=%s\n", args[2]);
                fclose(fp);
                chmod(sp_txt, 0644);
                ...
                strcpy(response, "ERROR: failed to open security_patch.txt for writing\n");
            }
        ...
        if (argc >= 2 && strcmp(args[1], "status") == 0) {
            ...
            if (!installed) {
                ...
                /* HMA-OSS's zygote service silently discards the entire config
                 * if configVersion != BuildConfig.CONFIG_VERSION (see HMAService.kt
                 * loadConfig()), so this must track the app's current CONFIG_VERSION
                 * (93 as of frknkrc44/HMA-OSS master) rather than an arbitrary value. */
                int config_version = 93;
                ...
                if (fp) {
                    ...
                    fclose(fp);
                    chmod(cfg_file, 0600);
                    /* Every other file in this service dir (filter_count.json,
                     * preset_cache_v2.json, the dir itself) is owned by
                     * system:system since HMA-OSS's zygote service runs as
                     * system, not root. Leaving config.json root:root makes
                     * it unreadable to that process, so the whole config is
                     * silently ignored even though it was written correctly. */
                    chown(cfg_file, 1000, 1000);
    ...
    } else if (strcmp(args[0], "kstat") == 0) {
        if (argc >= 3 && strcmp(args[1], "stat") == 0) {
            char resolved_pathname[PATH_MAX];
            struct stat sb;
            if (!realpath(args[2], resolved_pathname) || stat(resolved_pathname, &sb) != 0) {
                strcpy(response, "ERROR: failed to stat path\n");
            } else {
                snprintf(response, sizeof(response),
                         "{\n"
                         "  \"ino\": \"%lu\",\n"
                         ...
                         (long)sb.st_blksize,
                         (long)sb.st_atime, (unsigned long)sb.st_atim.tv_nsec,
                         (long)sb.st_mtime, (unsigned long)sb.st_mtim.tv_nsec,
                         (long)sb.st_ctime, (unsigned long)sb.st_ctim.tv_nsec);
            }
        } else {
            strcpy(response, "ERROR: unknown kstat command\n");
        }
    } else {
